<?php
    $id =  $_GET["data"];
    $query = "SELECT * FROM product WHERE ProductNo = '$id'";
    $cx = mysqli_connect("localhost","root","","app");
    $result = mysqli_query($cx, $query);
    $row = mysqli_fetch_assoc($result);
    echo "ต้องการลบสินค้า รหัสสินค้า : ". $row['ProductNo'] ."<br>ชื่อสินค้า : ". $row['ProductName'] ."<br> ราคาต่อหน่วย : ". $row['PricePerUnit']."<br>จำนวนสินค้า : ". $row['Qty'];
    mysqli_close($cx);
?>                             
<form method="get" action="delete_form.php">
    <input type="hidden" name="data" value="<?php echo $row['ProductNo']; ?>">
